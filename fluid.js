...
(function(w) {
    ...
    */
	...
	let initialCells = [];
	const RECOVERY_RATE = 0.01; //0 is no mouse velocity recovery, 1 is instant

	function resizeCanvasToGrid(){
		//get raw viewport size
		// 1) compute a new width/height that’s a multiple of 'resolution'
		...
		// 2) update your grid dimensions
		num_cols = canvas_width  / resolution;
		num_rows = canvas_height / resolution;
		// 3) re-build everything with your init() function
		init();
	}
	
	// called from init(), right after you populate vec_cells:
	function seedFlow() 
	{
		...
		// deep-clone for later reset
		initialCells = vec_cells.map(col =>
		col.map(cell => ({ xv: cell.xv, yv: cell.yv, pressure: cell.pressure }))
		);
	}
    ...
    */
    function init() {
		particles = [];      // clear out any old particles
		vec_cells = [];      // reset grid
    ...
    */
    function draw() {
        ...
        */
		...
		// ——— gentle restore toward initial seeded velocities ———
		for (let col = 0; col < num_cols; col++) {
			for (let row = 0; row < num_rows; row++) 
			{
				const cell = vec_cells[col][row];
				const init = initialCells[col][row];
				cell.xv += (init.xv - cell.xv) * RECOVERY_RATE;
				cell.yv += (init.yv - cell.yv) * RECOVERY_RATE;
			}
		}
    ...
    */
	...
	// kick things off _and_ listen for resizes
